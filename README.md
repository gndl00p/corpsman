...
It boots into **AID STATION** — read-only. The destructive subcommands aren't merely
refused, they aren't *registered*: they don't appear in `--help`, they don't parse, and
the MCP server doesn't advertise them.
...
A flag can be pasted from a wiki, recalled from shell history, inherited from a copied
script, or emitted by an LLM completing a command. A held physical keypress at a real
terminal can't arrive by any of those routes.

- **`stdin` must be a TTY or arming is impossible**, with no override. One rule that
  closes paste-injection, accidental automation, CI misfires, and every prompt-injection
  path through the MCP server simultaneously.
- **Held, not tapped** — 3 seconds in raw terminal mode. A hold can't be expressed in a
  pasted string.
- **Process-scoped**, never written to disk, never inherited, expires after 10 minutes.
- Bench automation uses a separate path requiring `CORPSMAN_DEVIL_DOC=1` set in the
  operator's shell rather than inside the script — so a copy-pasted command block can't
  self-arm, and a script in a repo can't arm itself on someone else's machine.
...
- **Errors are fatal to the verdict.** Any IO error, unwritable region, or media defect
  drops a sanitization result to `INCOMPLETE` or `DESTROY_REQUIRED`. A dying drive never
  earns a clean certificate.

## Health verdicts

Weighted toward the five SMART attributes Backblaze's fleet data found carry nearly all
the predictive signal: 5, 187, 188, 197, 198. Every verdict prints the attribute values
that produced it — no opaque scores.
...
Runs append to a hash-chained `~/.corpsman/ledger.jsonl`, so retroactive edits are
detectable. The chain head prints at the end of each run for recording out-of-band.
`doc ledger --verify` revalidates it. This is tamper-evident, not a cryptographic
signature, and does not claim to be.
